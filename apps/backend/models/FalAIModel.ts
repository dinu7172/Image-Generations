...
export class FalAIModel extends BaseModel {
    ...
    public async generateImage(prompt: string, tensorPath: string): Promise<any> {
    ...
    public async trainModel(zipUrl: string, triggerWord: string): Promise<any> {
        const { request_id, response_url } = await fal.queue.submit("fal-ai/flux-lora-general-training", {
            input: {
